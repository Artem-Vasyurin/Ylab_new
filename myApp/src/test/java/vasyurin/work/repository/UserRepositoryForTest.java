...
public class UserRepositoryForTest implements UserRepository {
    ...
    private void insert(User user) {
        try (Connection conn = connection.getConnection();
            ...
            ps.setString(3, user.getRole().name()); // теперь используем .name() для enum
        ...
        try (Connection conn = connection.getConnection();
            ...
            if (rs.next()) {
                return Optional.of(new User(
                        ...
                        UserRole.valueOf(rs.getString("role")), // конвертируем строку в enum
                        rs.getString("token")
                ));
            }
        ...
        try (Connection conn = connection.getConnection();
            ...
            while (rs.next()) {
                list.add(new User(
                        rs.getString("username"),
                        rs.getString("password"),
                        UserRole.valueOf(rs.getString("role")), // тоже enum
                        rs.getString("token")
                ));
            }
